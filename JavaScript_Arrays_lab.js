...
for (let i = 0; i < numArrays.length; i += 1) {

    // declaring a variable for storing the first loop iteration through the array
    const firstLoop = numArrays[i];
    
    for (let c = 0; c < firstLoop.length; c += 1) {

      //declaring a variable for storing the nested items inside the arrays
      const secondLoop = firstLoop[c];
      // console.log(secondLoop)
      
      //getting the sum of the nested items within the arrays and storing it in the total variable
      total =  total + secondLoop;
    }
}
